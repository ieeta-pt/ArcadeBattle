function GestureRecon(decision_trees, draw_p5js) {
    ...
    this.start = function() {
        ...
        this.paused = false;
        this.is_left_hand_open = false;
        this.left_gesture_done = false;
        this.left_gesture_time = undefined;
        this.is_right_hand_open = false;
        this.right_gesture_done = false;
        this.right_gesture_time = undefined;
        ...
        var predictions_left = [];
        var predictions_right = [];
        ...
        Leap.loop(this.controllerOptions, function(frame) {
            ...
            if (frame.hands.length == 2) {
                var hand_1 = vectorToArray(frame.hands[0].palmPosition);
                var hand_2 = vectorToArray(frame.hands[1].palmPosition);
                if (hand_1[0] < hand_2[0]) {
                    this.left_hand = frame.hands[0];
                    this.right_hand = frame.hands[1];
                } else {
                    this.left_hand = frame.hands[1];
                    this.right_hand = frame.hands[0];
                }
                //console.log("LEFT HAND: "+this.left_hand);
                //console.log("RIGHT HAND: "+this.right_hand);
            ...
            if (frame.pointables.length > 0) {
                ...
                var fingers_dist = {};
                var fingers_dist_left = {};
                var fingers_dist_right = {};
                fingers_dist_left["n_extended_fingers"] = 0;
                fingers_dist_right["n_extended_fingers"] = 0;
                var left_counter = 0;
                var right_counter = 0;
                var prev_left_finger = undefined;
                var prev_right_finger = undefined;
                for (var i = 0; i < frame.pointables.length; i++) {
                    ...
                    if (vectorToArray(pointable.hand().palmPosition)[0] > 0) {
                        if (right_counter == 0) {
                            prev_right_finger = vectorToArray(frame.pointables[i].tipPosition);
                            right_counter++;
                            continue;
                        }
                        curr_finger = vectorToArray(pointable.tipPosition);
                        var prev_finger = prev_right_finger;
                        fingers_dist_right["adj_fingers_dist"+right_counter] = Math.sqrt((prev_finger[0]-curr_finger[0])**2 + 
                                                                       (prev_finger[1]-curr_finger[1])**2 + 
                                                                       (prev_finger[2]-curr_finger[2])**2);
                        fingers_dist_right["n_extended_fingers"] += (pointable.extended ? 1 : 0);
                        right_counter++;
                        prev_right_finger = curr_finger;
                    } else {
                        if (left_counter == 0) {
                            prev_left_finger = vectorToArray(frame.pointables[i].tipPosition);
                            left_counter++;
                            continue;
                        }
                        curr_finger = vectorToArray(pointable.tipPosition);
                        var prev_finger = prev_left_finger;
                        fingers_dist_left["adj_fingers_dist"+left_counter] = Math.sqrt((prev_finger[0]-curr_finger[0])**2 + 
                                                                       (prev_finger[1]-curr_finger[1])**2 + 
                                                                       (prev_finger[2]-curr_finger[2])**2);
                        fingers_dist_left["n_extended_fingers"] += (pointable.extended ? 1 : 0);
                        left_counter++;
                        prev_left_finger = curr_finger;
                    }
                }
                if (this.option == "Test") {
                    prediction_left = (this.decision_trees[this.decision_tree_index-1].predict(fingers_dist_left) == 1 ? this.gesture_name : "None");
                    prediction_right = (this.decision_trees[this.decision_tree_index-1].predict(fingers_dist_right) == 1 ? this.gesture_name : "None");
                    this.predictions_left.push(prediction_left);
                    this.predictions_right.push(prediction_right);
                    if (this.predictions_left.length == 3) {
                        counters = {};
                        for (var pred = 0; pred < this.predictions_left.length; pred++) {
                            if (this.predictions_left[pred] in counters)
                                counters[this.predictions_left[pred]] += 1;
                            else
                                counters[this.predictions_left[pred]] = 0;
                        }
                        var max_value = this.predictions_left[0];
                        for (var key in counters) {
                            if (counters[key]  > counters[max_value])
                                max_value = key;
                        }
                        document.getElementById("this.gesture_name").innerHTML = max_value;
                        this.predictions_left = [];
                    }
                }
                else if (option == "websockets") {
                    prediction_left = undefined;
                    prediction_right = undefined;
                    for (var i = 0; i < decision_trees.length; i++) {
                        //console.log(fingers_dist_left)
                        //console.log(fingers_dist_right)
                        if (decision_trees[i].predict(fingers_dist_left) == 1 && prediction_left == undefined) {
                            prediction_left = decision_trees[i].name;
                        }
                        if (decision_trees[i].predict(fingers_dist_right) == 1 && prediction_right == undefined) {
                            prediction_right = decision_trees[i].name;
                        }
                    }
                    predictions_left.push(prediction_left);
                    predictions_right.push(prediction_right);
                    if (predictions_left.length == 10) {
                        counters = {};
                        for (var pred = 0; pred < predictions_left.length; pred++) {
                            if (predictions_left[pred] in counters)
                                counters[predictions_left[pred]] += 1;
                            else
                                counters[predictions_left[pred]] = 0;
                        }
                        var max_value = predictions_left[0];
                        for (var key in counters) {
                            if (counters[key]  > counters[max_value])
                                max_value = key;
                        }
                        try {
                            //console.log(max_value + " => " + this.is_hand_open);
                            if (this.left_gesture_done == undefined) this.left_gesture_done = false;
                            if (max_value == "open_hand") {
                                if (this.is_left_hand_open == false && !this.left_gesture_done)
                                    console.log("MISS!!!!!!");
                                this.is_left_hand_open = true;
                                this.left_gesture_done = false;
                            }
                            else {
                                if (this.left_gesture_done == false && max_value == "closed_hand") {
                                    websocket.send('{"gesture_recognized": "'+max_value+'_left"}');
                                    this.left_gesture_done = true;
                                    console.log("time : " + ((new Date()).getTime() - this.left_gesture_time));
                                    this.left_gesture_time = undefined;
                                }
                                if (this.left_gesture_time == undefined)
                                    this.left_gesture_time = (new Date()).getTime();
                                this.is_left_hand_open = false;
                            }
                    ...
                    if (predictions_right.length == 10) {
                        counters = {};
                        for (var pred = 0; pred < predictions_right.length; pred++) {
                            if (predictions_right[pred] in counters)
                                counters[predictions_right[pred]] += 1;
                            else
                                counters[predictions_right[pred]] = 0;
                        }
                        var max_value = predictions_right[0];
                        for (var key in counters) {
                            if (counters[key]  > counters[max_value])
                                max_value = key;
                        }
                        try {
                            //if (this.right_gesture_done == undefined) this.right_gesture_done = false;
                            //console.log("right: "+max_value + " => " + this.is_hand_open);
                            if (max_value == "open_hand") {
                                //console.log(this.is_right_hand_open + " , " + this.right_gesture_done);
                                if (this.is_right_hand_open == false && !this.right_gesture_done)
                                    console.log("MISS right!!!!!!");
                                this.is_right_hand_open = true;
                                this.right_gesture_done = false;
                            }
                            else {
                                if (this.right_gesture_done == false && max_value == "closed_hand") {
                                    websocket.send('{"gesture_recognized": "'+max_value+'_right"}');
                                    this.right_gesture_done = true;
                                    console.log("time : " + ((new Date()).getTime() - this.right_gesture_time));
                                    this.right_gesture_time = undefined;
                                }
                                if (this.right_gesture_time == undefined)
                                    this.right_gesture_time = (new Date()).getTime();
                                this.is_right_hand_open = false;
                            }
